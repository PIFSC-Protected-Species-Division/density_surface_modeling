...
**Note!!!** *Currently this is the way that PIFSC PSD has downloaded oceanographic model data from Copernicus Marine Service ([https://marine.copernicus.eu/](https://marine.copernicus.eu/)). This, more complicated, method was necessary because the `CopernicusMarine`package did not have the ability to download ocean product data subsets. However, as of September 2025 this capability was repaired. So, the `CopernicusMarine` package (available from [https://pepijn-devries.github.io/CopernicusMarine/](https://pepijn-devries.github.io/CopernicusMarine/)) is probably a more efficient way to download the data and we will investigate this further.*


The procedure for downloading the oceanographic prediction data is nearly identical to that in step 3. The only difference is specifying the spatial and temporal grid defined by the Central Pacific study region and the tri-daily grid of days from July 1 through December 31 in the years 2017 and 2020-2024. 
...
## Processing the raw netCDF files
...
r <- rast( file.path(pred_dir, paste0("cenpac_",years[5], "_env.tif")))
r <- r[[time(r)==as.Date("2023-07-31")]] %>% toMemory()
ppp <- vector("list",nlyr(r))
for(i in 1:nlyr(r)){
  ppp[[i]] <- ggplot() + layer_spatial(r[[i]]) + 
    # scale_fill_princess_c(palette = "maori", name=names(r)[i]) + 
    scale_fill_viridis(option="H", name=names(r)[i]) + 
    scale_x_continuous(breaks=c(175, 228)) + scale_y_continuous(breaks=c(0, 40)) + theme_bw()
  print(ppp[[i]])
}
```
...
Here, we load the survey segment data and sightings data from steps 1 and 2 and clean it up for DSM fitting with the `mgcv` package. 
...
Here, we add the augmented data for mixture-like weighting of encounters where the probability that the sighting is of pelagic stock animals is less than 1. For all sightings with `ProbPel` < 1, we add an additional line of data for each segment where the number of groups detected is `nSI_033 <- 0` and `ProbPel <- 1-ProbPel` for the observed encounter. 
...
The group size model set is relatively easy to for as it simply consists of every combination of the 4 oceanographic variables for a total of 16 models. 
...
### Evaluate ratio of design-based to model-based predicted density

Here we examine the ratio of density estimates on the survey segments between design and model based estimators. 
...
Menardi and Torelli (2014) note that when data are highly imbalanced, performance metrics, such as AUC and TSS, as well as parameter estimation, can be compromised. So, in addition to the AUC and TSS calculations, we also used the ROSE (Random Over-Sampling Examples) sampling bootstrap (Lunardon et al., 2014) to assess the effects that sparse detection of false killer whales might have had on AUC and TSS metrics for the just the encounter rate model as it is the portion that is severely imbalanced. 
 The ROSE sampling method is described in detail by Menardi and Torelli (2014), but briefly, kernel density estimates are calculated for segments with and without detections. Then a sample is drawn with probability = 0.5 from each KDE till the desired sample size is reached. This produces a balanced sample where the predictive covariates are similar to what was observed for each group (detection vs. nondetection) in the original data.

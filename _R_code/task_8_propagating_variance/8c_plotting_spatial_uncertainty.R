...
# Load survey data
load(here("_R_code","task_4_fit_dsm_models", "output","dsm_model_data.RData"))
surv_loc <- fkw_dsm_data %>% select(mlon,mlat) %>% 
  st_as_sf(coords=c("mlon","mlat"), crs=4326) %>% st_shift_longitude()

ppp <- vector("list",nlyr(cv_raster))

ppp[[1]] <- ggplot() + geom_spatraster(data=cv_raster[[1]]) +
  layer_spatial(surv_loc, shape=20, color=gray(0.8), size=0.01, alpha=0.25) + 
...
ppp[[1]] <- ggplot() + geom_spatraster(data=cv_clip[[1]]) +
  annotation_spatial(surv_loc, shape=20, color=gray(0.2), size=0.001, alpha=0.1) + 
